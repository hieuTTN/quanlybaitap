...
async function handleAddAccount(event) {
    ...
    const payload = {
        ...
        authorities: {name:event.target.elements.role.value},
    };
...
const AdminUser = ()=>{
    ...
    async function changeRole(id, rolename) {
        var response = await postMethod('/api/user/admin/change-role?id='+id+'&role='+rolename);
        if(response.status < 300){
                        ...
                        <tbody>
                            {items.map((item=>{
                                ...
                                return  <tr>
                                    ...
                                    <td> 
                                    <select onChange={(e)=>changeRole(item.id, e.target.value)} className='form-control' name='role'>
                                        <option selected={item.authorities?.name == 'ROLE_ADMIN'} value='ROLE_ADMIN'>Quản trị</option>
                                        <option selected={item.authorities?.name == 'ROLE_TEACHER'} value='ROLE_TEACHER'>Giảng viên</option>
                                        <option selected={item.authorities?.name == 'ROLE_STUDENT'} value='ROLE_STUDENT'>Sinh viên</option>
                                    </select>
                                    </td>
                                    <td class="sticky-col">
                                        {btn}
